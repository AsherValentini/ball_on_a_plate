...
# Example PID update function, including clipping to angle range
def pid_update(error, dt, pid_coefficients, angle_range, axis):
    global previous_error_x, previous_error_y, integral_x, integral_y
    Kp, Ki, Kd = pid_coefficients

    # Selecting the correct axis
    previous_error = previous_error_x if axis == 'x' else previous_error_y
    integral = integral_x if axis == 'x' else integral_y
...
while(True):
    ...
    if valid_balls:
        ...
        pid_coefficients = (1.0, 0.0, 0.1)  # Example PID coefficients
        ...
        # PID update and get angle adjustments
        angle_adjust_x = pid_update(error_x, dt, pid_coefficients, angle_range, 'x')
        angle_adjust_y = pid_update(error_y, dt, pid_coefficients, angle_range, 'y')
