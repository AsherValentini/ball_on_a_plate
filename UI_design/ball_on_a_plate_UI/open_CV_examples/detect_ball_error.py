...
class MyThread(QThread):
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if ret:
                ...
                # Emit the ball's position (error_x, error_y) if the ball was detected
                if error_x is not None and error_y is not None:
                    self.position_signal.emit(error_x, error_y)
            else:
    ...
    def detect_yellow_ball(self, image):
        ...
        frame_center_x = image.shape[1] // 2 -7 # X center of the frame
        frame_center_y = image.shape[0] // 2 +17 # Y center of the frame
        ...
        if contours:
            ...
            if cv2.contourArea(largest_contour) > 500:
                ...
                # Calculate error as the difference from the frame's center
    ...
    def cvimage_to_label(self, image):
        image = imutils.resize(image, width=640, height=800)
...
class MainApp(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.camera_thread.position_signal.connect(self.update_position)  # Connect to receive position updates
    ...
    def update_position(self, error_x, error_y):
        print(f"Ball position relative to center: Error X: {error_x}, Error Y: {error_y}")
        # You can add further logic to process the position errors and send them to the motors
